...
void loadModel(std::vector<Vertex> &model, MODEL_TYPE &m_lastModel, GLuint &tex, GLuint &nor) {
	std::string path = "cube.obj";
	model.clear();
	if (m_currentModel == CUBE) {
		path = "cube.obj";
		tex = loadImage("textures/cube_diffuse.png");
		nor = loadImage("textures/cube_normal.png");
		m_lastModel = CUBE;
	}
	else if (m_currentModel == CYBORG) {
		path = "cyborg.obj";
		tex = loadImage("textures/cyborg_diffuse.png");
		nor = loadImage("textures/cyborg_normal.png");
		m_lastModel = CYBORG;
	}
	path = "models/" + path;
	load_obj(path, model);
